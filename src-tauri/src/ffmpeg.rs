...
impl FfmpegEncoder {
    ...
    fn detect_encoder(ffmpeg_path: &PathBuf) -> (String, Vec<String>) {
        ...
        let candidates = [
            ...
            ), // AMD Radeon (Simplificado para integrado)
        ...
        for (codec, preset) in candidates {
            if Self::test_encoder(ffmpeg_path, codec) {
                let success_msg = format!(
                    "🚀 GPU Encoder Confirmed: Using {} with preset {:?}\n",
                    codec, preset
                );
                println!("{}", success_msg);
            ...
            Ok(out) => {
                if out.status.success() {
                    ...
                    if !err.is_empty() && !err.contains("deprecated") {
                        let reason =
                            format!("      Reason: {}", err.trim().lines().next().unwrap_or(""));
                        println!("{}", reason);
    ...
    fn spawn_process(
        ...
        println!(
            "📹 HLS Config: list_size={}, hls_time={}, output={}",
            list_size, hls_time, output_path
        );
        ...
        if enable_audio && enable_microphone {
            ...
            command.args([
                ...
                &audio_buffer_size.to_string(),
                "-i",
                &format!("audio={}", microphone_device),
            ]);

            // Simplificado: usar amix para mixagem automática (resampling + channel layout)

            let sys_filter_str = if !audio_filters.is_empty() {
                format!(
            ...
            let mic_filter_str = if !microphone_filters.is_empty() {
                format!(
                    "aresample=48000:async=1,asetpts=PTS-STARTPTS,{}",
                    microphone_filters
                )
            ...
            println!("🎤🔊 Mixing Audio (Simplified): System + Mic (amix)");
            ...
            println!(
                "🔊 System Audio Only: device='{}' (offset: {}ms)",
                audio_device, audio_offset
            );
            ...
            println!(
                "🎤 Microphone Only: device='{}' (offset: {}ms)",
                microphone_device, audio_offset
            );
        } else {
        ...
        command.args([
            ...
            &format!("{}k", bitrate / 1000),
            "-bufsize",
            &format!("{}k", bitrate / 500),
            "-g",
            &fps.to_string(), // GOP size = 1s
            "-color_range",
            "tv",
            "-colorspace",
            "bt709",
        ...
        let needs_af_filter = (enable_audio || enable_microphone)
            && !(enable_audio && enable_microphone)
            && !audio_filters.is_empty();
        ...
        command.args([
            ...
            "2", // Stereo
            "-aac_coder",
            "fast", // 'fast' é mais estável que 'twoloop' para sinais complexos
        ]);

        // Apply audio filters for single input streams (system OR mic, not both)
        if needs_af_filter {
            // Adiciona aresample para garantir consistência de 48kHz vindo do WASAPI + Sync + PTS Reset
            let final_filters = format!(
                "aresample=48000:async=1,asetpts=PTS-STARTPTS,{}",
                audio_filters
            );
            ...
            // Mesmo sem filtros de efeito, garante o resample + Sync + PTS Reset
            command.args(["-af", "aresample=48000:async=1,asetpts=PTS-STARTPTS"]);
        }
        ...
        println!(
            "FFmpeg started: {} ({}x{} @ {} fps) [Encoder: {}]",
            stream_id, width, height, fps, codec
        );
